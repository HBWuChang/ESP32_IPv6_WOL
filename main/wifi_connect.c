...
esp_err_t example_wifi_connect(const char *wifi_ssid, const char *wifi_passwd)
{
    ...
    wifi_config_t wifi_config = {
        .sta = {
            ...
            .threshold.authmode = WIFI_AUTH_OPEN, // 默认设置为开放模式
        },
    };

    // 设置 SSID
    strncpy((char *)wifi_config.sta.ssid, wifi_ssid, sizeof(wifi_config.sta.ssid) - 1);
    wifi_config.sta.ssid[sizeof(wifi_config.sta.ssid) - 1] = '\0'; // 确保以 null 结尾

    // 判断密码是否为空
    if (wifi_passwd && strlen(wifi_passwd) > 0)
    {
        strncpy((char *)wifi_config.sta.password, wifi_passwd, sizeof(wifi_config.sta.password) - 1);
        wifi_config.sta.password[sizeof(wifi_config.sta.password) - 1] = '\0'; 
        wifi_config.sta.threshold.authmode = EXAMPLE_WIFI_SCAN_AUTH_MODE_THRESHOLD; 
    }
    else
    {
        ESP_LOGW(TAG, "WiFi password is empty, using open authentication mode.");
    }
